function lasso_sparsity

close all hidden

% center of ellipse not too close to origin
p = (rand(2,1)-.5)*5;
while norm(p)<1.5
  p = (rand(2,1)-.5)*5;
end
d = rand(2,1)+1; % major and minor axes length
ang = rand*2*pi;  % rotation
...
sld = uislider(fig,...
  ...
  'ValueChangingFcn',@(sld,event) sliderMoving(ax,p,d,ang,event));
sld.Limits = [0 4];
ball1(ax,1);
axis(ax,'equal');
axis(ax,[-3 3 -3 3]);
...
function sliderMoving(ax,p,d,ang,event)
cla(ax)
ball1(ax,1);
ellipse(ax,d(1)*event.Value,d(2)*event.Value,ang,p(1),p(2),[],[]);
axis(ax,'equal');
axis(ax,[-3 3 -3 3]);
...
function h=ellipse(ax,ra,rb,ang,x0,y0,C,Nb)
...
if isempty(C),C=get(ax,'colororder');end;
